...
export default function InboxTebusan() {
  ...
  const { data: items = [], isLoading } = useQuery({
    queryKey: ["inbox-tebusan", profile?.division_id, isSuperAdmin],
    queryFn: async () => {
      ...
      return data.filter(s => {
        const tebusan = s.tebusan as string[];
        return profile?.division_id && tebusan.includes(profile.division_id);
      });
    },
    enabled: !!profile,
  });
      ...
      <div className="space-y-6">
        ...
        <div className="bg-card border rounded-lg p-6 space-y-4">
          <div className="flex items-start justify-between">
            ...
            <Badge className="bg-blue-100 text-blue-800">Tebusan</Badge>
          </div>
          ...
          <div className="pt-2 border-t">
            <p className="text-sm text-muted-foreground italic">Anda menerima surat ini sebagai tebusan (CC). Hanya dapat dilihat, tidak dapat diubah.</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Tebusan Surat</h1>
        <p className="text-muted-foreground mt-1">Surat internal di mana divisi Anda tercantum sebagai tebusan (CC).</p>
      </div>
      <DataTable
        data={items}
        isLoading={isLoading}
        searchPlaceholder="Cari tebusan surat..."
